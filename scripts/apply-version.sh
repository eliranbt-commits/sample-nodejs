#!/usr/bin/env bash
# Write the release version into the GitOps chart (Chart.yaml + values-gitops.yaml).
set -euo pipefail

version="${1:?usage: apply-version.sh <x.y.z> <image-repository> <gitops-root>}"
repository="${2:?usage: apply-version.sh <x.y.z> <image-repository> <gitops-root>}"
gitops_root="${3:?usage: apply-version.sh <x.y.z> <image-repository> <gitops-root>}"
version="${version//$'\r'/}"
version="${version## }"
version="${version%% }"
repository="${repository//$'\r'/}"

chart="$gitops_root/helm/sample-nodejs/Chart.yaml"
values="$gitops_root/helm/sample-nodejs/values-gitops.yaml"
